...
let rec scomp expr =
  ...
  | OpUna(op,x) ->
    match op with
    | Neg -> (scomp x)@[SNeg]
    | Not -> (scomp x)@[SIfze(3)]@[SCst(0)]@[SMul]@[SJump(2)]@[SCst(1)]//ifz3,*,0,j1,1 have yet to test; tested and updated
  | OpBin(op,xp1,xp2) ->
    match op with
...
//2,4,4,-,if0,4,*,0,j,1,1,*
//2
let testSourceExpression3 = OpBin(Add,Cst(2),OpUna(Not,OpBin(Sub,Cst(4),Cst(4))))
//2*eq(4,4); 244eq*
//x1,x2,-,if0,
//02
let testSourceExpression4 = OpBin(Mul,Cst(2),OpBin(Eq,Cst(2),OpBin(Mul,Cst(2),Cst(1))))

//2*and(1,5); 215and*
//eq(2,not(eq(5,0))+not(eq(1,0)))
//512
let testSourceExpression5 = OpBin(Mul,Cst(2),OpBin(And,Cst(1),Cst(9)))
...
printfn "Compiled to Stack: %A" (scomp testSourceExpression3) //23*4+
printfn "Test Source Expression 4: %A" testSourceExpression4
printfn "Compiled to Stack: %A" (scomp testSourceExpression4) //23*4+
printfn "Test Source Expression 5: %A" testSourceExpression5
printfn "Compiled to Stack: %A" (scomp testSourceExpression5) //23*4+
